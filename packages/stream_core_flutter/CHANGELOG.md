...
- Added `StreamSafeArea` — a `SafeArea` variant that floats its child a fixed per-side `margin` *beyond* the system insets instead of flooring at them (`SafeArea` pads to `max(inset, minimum)`; this pads to `inset + margin`), so a pinned surface keeps a consistent gap above the status bar / navigation bar / home indicator rather than sitting flush. It reads `viewPadding` (not `padding`), so the gap survives an open keyboard, and `avoidKeyboard: true` additionally clears the keyboard. Resolve the same insets as a value — e.g. to size a gradient behind the child — with the static `StreamSafeArea.resolveInsets`. Used internally by the floating `StreamBottomNavBar` pill.
