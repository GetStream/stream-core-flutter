...
- Added `StreamSafeArea` — a `SafeArea` variant that floats its child a fixed per-side `margin` *beyond* the system insets instead of flooring at them (`SafeArea` pads to `max(inset, minimum)`; this composes it and pads to `inset + margin`), so a pinned surface keeps a consistent gap above the status bar / navigation bar / home indicator rather than sitting flush. Like `SafeArea.maintainBottomViewPadding`, the bottom gap is measured from `viewPadding`, so it stays put when a keyboard covers it. Resolve the same insets as a value — e.g. to size a gradient behind the child — with the static `StreamSafeArea.resolveInsets`. Used internally by the floating `StreamBottomNavBar` pill.
