...
- Added `StreamSafeArea` — a `SafeArea` variant that insets its child by `max(systemInset, minimum) + margin` on each edge: `minimum` floors an edge (like `SafeArea.minimum`) while `margin` adds beyond the safe area, so a pinned surface keeps a controlled gap from the status bar / navigation bar / home indicator rather than sitting flush. The bottom gap is measured from `padding` by default, matching `SafeArea`; set `maintainBottomViewPadding: true` to measure it from `viewPadding` instead so a pinned surface stays put when a keyboard covers it. Resolve the same insets as a value — e.g. to size a gradient behind the child — with the static `StreamSafeArea.resolveInsets`. `StreamSafeArea.driven` interpolates the inset toward a target (default `EdgeInsets.zero`) driven by any `ValueListenable<double>`, for a floating surface that releases its space as a panel slides in beneath it. Used internally by the floating `StreamBottomNavBar` pill.
- Added `StreamAppBar.resolveSurfaceStyle`, `StreamBottomAppBar.resolveSurfaceStyle`, and `StreamBottomNavBar.resolveSurfaceStyle` — statics that report the surface style a bar renders with in a given context, resolving the per-instance `style`, then the bar's component theme, then the ambient `StreamSurfaceStyle`. Pass the result as `StreamScaffold.bottomSurfaceStyle` when a bar's own theme pins a style, so the scaffold's inset matches what the bar draws.
