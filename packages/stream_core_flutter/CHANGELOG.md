...
- Added `StreamScaffold` — a full-page scaffold for regular or floating bars; a floating bar enlarges the body's `MediaQuery.padding` by its extent so scrollables inset themselves.
...
- Added `StreamSafeArea` — a `SafeArea` that insets by `max(systemInset, minimum) + margin`, so a pinned surface keeps a gap from the system bars instead of sitting flush. `StreamSafeArea.driven` animates the inset; `StreamSafeArea.resolveInsets` returns it as a value.
- Added `resolveSurfaceStyle` statics to `StreamAppBar`, `StreamBottomAppBar`, and `StreamBottomNavBar`, reporting the surface style a bar renders with so a container can lay out its slot to match.
...
- Each bar publishes its resolved surface style to a `StreamToolbarScope` around its slots, so slot widgets match the bar. Read it with `StreamToolbarScope.of`.
- Added `StreamToolbarButton` — a toolbar action (labelled, or icon-only via `StreamToolbarButton.icon`) that takes its look from the enclosing `StreamToolbarScope`; pass `type` to override the resolved shape.
- `StreamMediaViewer` chrome now follows the ambient `StreamSurfaceStyle` — floating over full-bleed media, or docked with the media inset between the bars. Pin it per-viewer via `StreamMediaViewerThemeData`.
