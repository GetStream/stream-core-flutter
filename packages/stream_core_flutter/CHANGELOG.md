...
- Added `isFloating` to `StreamAvatar`, `StreamAvatarGroup`, and `StreamAvatarStack`. When true, the avatar renders with a Material elevation (`StreamAvatarThemeData.floatingElevation`, defaulting to `StreamElevation.level2`) instead of a hand-painted shadow, so it matches the shadow of any other elevated surface next to it. Setting `isFloating` on a group or stack enables floating for every child avatar at once.
