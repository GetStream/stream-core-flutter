...
const std::string& ThumbnailRequest::video() const {
  return video_;
}

void ThumbnailRequest::set_video(std::string_view value_arg) {
  video_ = value_arg;
}


const EncodableMap* ThumbnailRequest::headers() const {
...
const ThumbnailFormat& ThumbnailRequest::format() const {
  return format_;
}

void ThumbnailRequest::set_format(const ThumbnailFormat& value_arg) {
  format_ = value_arg;
}


int64_t ThumbnailRequest::max_height() const {
  return max_height_;
}

void ThumbnailRequest::set_max_height(int64_t value_arg) {
  max_height_ = value_arg;
}


int64_t ThumbnailRequest::max_width() const {
  return max_width_;
}

void ThumbnailRequest::set_max_width(int64_t value_arg) {
  max_width_ = value_arg;
}


int64_t ThumbnailRequest::time_ms() const {
  return time_ms_;
}

void ThumbnailRequest::set_time_ms(int64_t value_arg) {
  time_ms_ = value_arg;
}


int64_t ThumbnailRequest::quality() const {
  return quality_;
}

void ThumbnailRequest::set_quality(int64_t value_arg) {
  quality_ = value_arg;
}
...
EncodableList ThumbnailRequest::ToEncodableList() const {
  ...
  list.push_back(EncodableValue(time_ms_));
...
size_t ThumbnailRequest::Hash() const {
  size_t result = 1;
  ...
  result = result * 31 + PigeonInternalDeepHash(time_ms_);
...
std::ostream& operator<<(
  std::ostream& os,
  const ThumbnailRequest& obj) {
  os << "ThumbnailRequest(";
  os << "video: ";
  os << PigeonInternalToString(obj.video_);
  os << ", headers: ";
  if (obj.headers_) {
  ...
  os << ", max_height: ";
  os << PigeonInternalToString(obj.max_height_);
  os << ", max_width: ";
  os << PigeonInternalToString(obj.max_width_);
  os << ", time_ms: ";
  os << PigeonInternalToString(obj.time_ms_);
  os << ", quality: ";
...
size_t PigeonInternalDeepHash(const ThumbnailRequest& v) {
  return v.Hash();
}
...
EncodableValue PigeonInternalCodecSerializer::ReadValueOfType(
  ...
  switch (type) {
    case 129: {
        ...
        return encodable_enum_arg.IsNull() ? EncodableValue() : CustomEncodableValue(static_cast<ThumbnailFormat>(enum_arg_value));
      }
    case 130: {
        return CustomEncodableValue(ThumbnailRequest::FromEncodableList(std::get<EncodableList>(ReadValue(stream))));
      }
    default:
...
void PigeonInternalCodecSerializer::WriteValue(
  ...
  if (const CustomEncodableValue* custom_value = std::get_if<CustomEncodableValue>(&value)) {
    if (custom_value->type() == typeid(ThumbnailFormat)) {
      stream->WriteByte(129);
      WriteValue(EncodableValue(static_cast<int>(std::any_cast<ThumbnailFormat>(*custom_value))), stream);
      return;
    }
    if (custom_value->type() == typeid(ThumbnailRequest)) {
      stream->WriteByte(130);
      WriteValue(EncodableValue(std::any_cast<ThumbnailRequest>(*custom_value).ToEncodableList()), stream);
      return;
    }
  }
...
const ::flutter::StandardMessageCodec& StreamThumbnailHostApi::GetCodec() {
...
void StreamThumbnailHostApi::SetUp(
  ::flutter::BinaryMessenger* binary_messenger,
  StreamThumbnailHostApi* api) {
  StreamThumbnailHostApi::SetUp(binary_messenger, api, "");
}
...
void StreamThumbnailHostApi::SetUp(
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.stream_thumbnail.StreamThumbnailHostApi.thumbnailData" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const ::flutter::MessageReply<EncodableValue>& reply) {
        try {
          ...
          const auto& request_arg = std::any_cast<const ThumbnailRequest&>(std::get<CustomEncodableValue>(encodable_request_arg));
          api->ThumbnailData(request_arg, [reply](ErrorOr<std::vector<uint8_t>>&& output) {
            if (output.has_error()) {
  ...
  {
    BasicMessageChannel<> channel(binary_messenger, "dev.flutter.pigeon.stream_thumbnail.StreamThumbnailHostApi.thumbnailFile" + prepended_suffix, &GetCodec());
    if (api != nullptr) {
      channel.SetMessageHandler([api](const EncodableValue& message, const ::flutter::MessageReply<EncodableValue>& reply) {
        try {
          const auto& args = std::get<EncodableList>(message);
          ...
          const auto& request_arg = std::any_cast<const ThumbnailRequest&>(std::get<CustomEncodableValue>(encodable_request_arg));
          api->ThumbnailFile(request_arg, [reply](ErrorOr<std::string>&& output) {
            if (output.has_error()) {
              reply(WrapError(output.error()));
              return;
            }
...
EncodableValue StreamThumbnailHostApi::WrapError(std::string_view error_message) {
