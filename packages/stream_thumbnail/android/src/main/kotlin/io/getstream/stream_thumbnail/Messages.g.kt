...
enum class ThumbnailFormat(val raw: Int) {
  JPEG(0),
  PNG(1),
  WEBP(2);

  companion object {
    fun ofRaw(raw: Int): ThumbnailFormat? {
...
data class ThumbnailRequest (
...
)
 {
  ...
  override fun hashCode(): Int {
    ...
    result = 31 * result + MessagesPigeonUtils.deepHash(this.timeMs)
    ...
    return when (type) {
      129.toByte() -> {
        return (readValue(buffer) as Long?)?.let {
          ThumbnailFormat.ofRaw(it.toInt())
        }
      }
      130.toByte() -> {
...
interface StreamThumbnailHostApi {
  /** Generates a thumbnail for [ThumbnailRequest.video] and returns its bytes. */
  fun thumbnailData(request: ThumbnailRequest, callback: (Result<ByteArray>) -> Unit)
  /**
   * Generates a thumbnail for [ThumbnailRequest.video] and returns the path
   * it was written to.
   */
  fun thumbnailFile(request: ThumbnailRequest, callback: (Result<String>) -> Unit)
  ...
  companion object {
    ...
    @JvmOverloads
    fun setUp(binaryMessenger: BinaryMessenger, api: StreamThumbnailHostApi?, messageChannelSuffix: String = "") {
      val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.stream_thumbnail.StreamThumbnailHostApi.thumbnailData$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val requestArg = args[0] as ThumbnailRequest
            api.thumbnailData(requestArg) { result: Result<ByteArray> ->
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.stream_thumbnail.StreamThumbnailHostApi.thumbnailFile$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val requestArg = args[0] as ThumbnailRequest
            api.thumbnailFile(requestArg) { result: Result<String> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(MessagesPigeonUtils.wrapError(error))
              } else {
