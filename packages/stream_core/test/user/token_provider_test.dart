...
void main() {
  group('UserToken.anonymous', () {
    ...
    test('rejects a raw value claiming a real user', () {
      // An anonymous token must not stand in for a named user.
      expect(() => UserToken.anonymous(rawValue: generateTestJwt('alice')), throwsArgumentError);
    });

    test('rejects a raw value that is not a JWT at all', () {
      expect(() => UserToken.anonymous(rawValue: 'not-a-jwt'), throwsArgumentError);
    });

    test('rejects a raw value whose segments are not valid base64', () {
      // Shaped like a JWT, so parsing gets further before failing.
      expect(() => UserToken.anonymous(rawValue: 'a.b.c'), throwsFormatException);
    });
  });
  ...
  group('DynamicTokenProvider', () {
    test('returns JWT tokens from the loader', () async {
      final provider = TokenProvider.dynamic((userId) async => generateTestUserToken(userId));
    ...
    test('throws when the loader returns a non-JWT token', () {
      final provider = TokenProvider.dynamic((_) async => UserToken.anonymous());

      // The type is checked first, so this reports the wrong type rather than the id an anonymous
      // token happens to carry.
      expect(
        () => provider.loadToken('user-1'),
        throwsA(isA<ArgumentError>().having((it) => it.message, 'message', contains('Token type mismatch'))),
      );
    });

    test('throws when the loader returns a token for a different user', () {
      // Caching it would authenticate every later request as that user.
      final provider = TokenProvider.dynamic((_) async => generateTestUserToken('someone-else'));
