...
void main() {
  group('LocationCoordinate', () {
    ...
    test('distinguishes coordinates that differ below a centimetre', () {
      const location1 = LocationCoordinate(
        latitude: 37.7749,
        longitude: -122.4194,
      );

      // Closer together than any GPS fix can resolve, but not the same point.
      const location2 = LocationCoordinate(
      ...
      expect(location1.distanceTo(location2), greaterThan(0));
    });

    test('is usable as a Set element and a Map key', () {
      // Equality that compared within a tolerance could not be transitive, so
      // it could not agree with `hashCode` and these would both fail.
      final coordinates = [
        const LocationCoordinate(latitude: 37.7749, longitude: -122.4194),
        const LocationCoordinate(latitude: 37.7749, longitude: -122.4194),
      ];

      expect(coordinates.toSet(), hasLength(1));
      expect({coordinates.first: 'sf'}[coordinates.last], equals('sf'));
    });
  });
