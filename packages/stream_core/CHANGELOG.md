...
- Removed `AttachmentUploadException`: `StreamAttachmentUploader.upload` reports the upload's own failure unwrapped, and `uploadBatch` emits `(attachmentId, result)` records so which attachment failed travels beside the outcome rather than inside it
